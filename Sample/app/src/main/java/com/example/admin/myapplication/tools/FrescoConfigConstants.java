...
import com.example.admin.myapplication.view.InstrumentedDraweeView;
...
//    		  .setDiskTrimmableRegistry(diskTrimmableRegistry)//类将包含一个注册表的缓存减少磁盘空间的环境。
                .setMaxCacheSize(FrescoConfigConstants.MAX_DISK_CACHE_SIZE)//默认缓存的最大大小。10.24K的图片能放500张
                .setMaxCacheSizeOnLowDiskSpace(MAX_SMALL_DISK_LOW_CACHE_SIZE)//当设备的磁盘空间很少时，缓存的最大大小
                .setMaxCacheSizeOnVeryLowDiskSpace(MAX_SMALL_DISK_VERYLOW_CACHE_SIZE)//当设备的磁盘空间很少很少时,缓存的最大大小
//    		  .setversion(version)
                .build();
...
//			  .setLowResImageRequest(ImageRequest.fromUri(lowResUri))//先下载显示低分辨率的图
    ...
    public static void init(final Resources resources) {
        if (sPlaceholderDrawable == null) {
            sPlaceholderDrawable = resources.getDrawable(R.color.text_yellow);
        }
        if (sErrorDrawable == null) {
            sErrorDrawable = resources.getDrawable(R.color.text_gray);
        }
    }
